...
impl SslConfigCtx {
    ...
    pub(super) fn cmd(&mut self, cmd: &str, value: &str) -> c_int {
        ...
        match (command.action)(self, value) {
            Err(mut e) => {
                if self.flags.is_show_errors() {
                    e = Error::bad_data(&format!("cmd={cmd} value={value}")).raise()
                }
                e.into()
            }
    ...
    fn min_protocol(&mut self, proto: &str) -> Result<ActionResult, Error> {
    ...
    fn parse_protocol_version(proto: &str) -> Option<u16> {
        Some(match proto {
            "None" => 0,
            "TLSv1.2" => u16::from(ProtocolVersion::TLSv1_2),
            "TLSv1.3" => u16::from(ProtocolVersion::TLSv1_3),
            _ => return None,
        })
    }
...
type CommandAction = fn(&mut SslConfigCtx, value: &str) -> Result<ActionResult, Error>;
